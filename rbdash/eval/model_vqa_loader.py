...
class CustomDataset(Dataset):
    def __init__(
        self, questions, image_folder, tokenizer, image_processor, image_processor_aux, model_config
    ):
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self.model_config, "image_size_aux"):
            ...
            self.image_processor.size[
                "shortest_edge"
            ] = self.model_config.image_size_aux
            self.image_processor_aux.crop_size["height"] = self.model_config.image_size_aux
            self.image_processor_aux.crop_size["width"] = self.model_config.image_size_aux
        ...
        if hasattr(self.model_config, "image_size_aux"):
            ...
            if self.image_processor is not self.image_processor_aux:
                image_tensor_aux = process_images([image], self.image_processor_aux, self.model_config)[
                    0
                ]
            else:
...
def create_data_loader(
    ...
    dataset = CustomDataset(
        questions, image_folder, tokenizer, image_processor, image_processor_aux, model_config
    )
...
def eval_model(args):
    ...
    tokenizer, model, image_processor, image_processor_aux, context_len = load_pretrained_model(
        model_path, args.model_base, model_name, load_8bit=args.load_8bit
    )
    ...
    data_loader = create_data_loader(
        questions, args.image_folder, tokenizer, image_processor, image_processor_aux, model.config
    )

    for (input_ids, image_tensor, image_tensor_aux), line in tqdm(
            ...
            with torch.inference_mode():
                output_ids = model.generate(
                    ...
                    images=image_tensor.to(
                        dtype=model.dtype, device=model.device, non_blocking=True
                    ),
                    images_aux=image_tensor_aux.to(
                        dtype=model.dtype, device=model.device, non_blocking=True
                    )
